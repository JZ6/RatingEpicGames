import type { SteamRating, SteamInfo, HltbInfo, MetacriticInfo, EpicInfo } from "../types";
import { getHltbHours, formatFreeDate } from "../types";
...
export function SteamBadge({ info }: { info?: SteamInfo }) {
  ...
  const url = info.steam_url || (info.appid ? `https://store.steampowered.com/app/${info.appid}` : undefined);
  const inner = (
    <div className="sc">
...
export function EpicDateBadge({ info }: { info?: EpicInfo }) {
  ...
  const tip = count > 1
    ...
    : undefined;
  return (
    <span className="epic-date" data-tip={tip} tabIndex={tip ? 0 : undefined}>
      {label}
